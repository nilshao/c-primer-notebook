...
空语句只有一个单独的分号，如果程序的某个地方语法上需要一条语句但是逻辑上不需要，此时应该使用空语句。使用空语句时应该加上注释，从而令读这段代码的人知道该语句是有意省略的。
不要漏写分号也不要多写分号。

##### 复合语句(块)

复合语句是指用花括号括起来的，可能为空的，语句和声明的序列。复合语句被称为块，一个块就是一个作用域，在块中引入的名字只能在块内部以及嵌套在块中的子块访问。

如果在程序某个地方，语法上需要一条语句，但是逻辑上需要多条，则应该使用复合语句。如while,for等。也可以有空块，内部没有任何语句的一堆花括号，空块等价于空语句。

### 5.2. 语句作用域

可以在if, switch, while, for语句的控制结构内定义变量。定义在控制结构当中的变量只在相应语句内部中可见，一旦语句结束，变量也就超出其作用范围。

### 5.3. 条件语句

if语句，根据条件决定控制流；switch语句，他计算一个整型表达式的值，然后根据这几个值从几条执行路径中选择一条。

#### 5.3.1. if语句

if statement的作用是，判断一个指定的条件是否为真，根据判断结果决定是否执行另外一条语句。if包含两种形式，一种含有else分支，另一种没有。简单if语句的语法形式是
...
在这两个版本的if语句中，condition都必须用圆括号包围起来。condition可以是一个表达式，也可以是一个初始化了的变量声明。不管是表达时还是变量，其类型都必须能转换成布尔类型。通常情况下，statement1和statement2是块语句。

嵌套if语句：else if语句

悬垂else：当一个if嵌套在另一个if语句内部时，很可能if分支多余else分支，如何知道某个给定的else和哪个if匹配呢？这个问题称为悬垂else，C++中，规定else与离他最近的尚未匹配的if匹配，从而消除程序的二义性。
...
break语句用于终止离它最近的while, do while, for, switch语句，并从这些语句之后的第一条语句开始继续执行。

#### 5.5.2. continue语句

continue语句用于终止最近的循环中的当前迭代并立即开始下一次迭代。continue只能出现在for, while, do while循环的内部，或者嵌套在此类循环里的语句或块的内部。continue语句中断当前的迭代，但是仍然继续执行循环。
...
当程序某部分检测到一个他无法处理的问题时，需要用到异常处理，此时，检测出问题的部分应该发出某种信号以表明程序遇到了故障，无法继续下去了，而且信号的发出方无需知道故障将在何处得到解决。一旦发出异常信号，检测出问题的部分也就完成了任务。

异常处理机制为程序中异常检测和异常处理两部分的协作提供支持，C++中的异常处理包括

+ throw表达式，异常检测部分使用throw表达式来表示它遇到了无法处理的问题，我们说throw引发了异常
+ try语句块，异常处理部分使用try语句块处理异常，try语句块以关键字try开始，并以一个或多个catch子句结束。try语句块中代码抛出的异常通常会被某个catch子句处理，因为catch子句处理异常，所以也被称作异常处理代码。
...
实参是形参的初始值，第一个实参初始化第一个形参，第二个实参初始化第二个形参，以此类推。尽管实参与形参存在对应关系，但是没有规定实参的求值顺序，编译器可以任意可行的顺序对实参求值。
